...
export function BundleTimeline({ bundle }: { bundle: Bundle }) {
  return (
    <div className="p-6">
      <div className="relative pl-6 border-l border-white/10 space-y-8">
        ...
        {bundle.status === "completed" && (
          <motion.div
            initial={{ opacity: 0, x: -10 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: bundle.timeline.length * 0.1 }}
            className="relative"
          >
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
              <p className="text-emerald-400 font-bold text-sm">Bundle Operation Completed</p>
            </div>
          </motion.div>
        )}
      </div>
